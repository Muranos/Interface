...
function DSH:UpdateShardButtons()
	...
	for _, name in pairs(shardOrder) do
		if DSH.shardsInBags[name] then
			DSH:UpdateShardButton(buttonCount, DSH.shardsInBags[name].itemLink, DSH.shardsInBags[name].itemID)
			buttonCount = buttonCount + 1
		end
	end
end

function DSH:UpdateShardButton(i, itemLink, itemID)
	...
	shardButton.tex:SetTexture(itemIcon)
